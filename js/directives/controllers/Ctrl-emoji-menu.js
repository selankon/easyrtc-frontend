...
emojiMenu.controller ('emojiMenuCntrl', ['$scope', 'emojiCallback',
  function($scope, emojiCallback) {
    ...
    // THANKS TO: https://github.com/globaldev/angular-emoji-filter/blob/master/lib/emoji.js
    $scope.emojisList = [
            ...
            $scope.filteredEmojis = []
              ,$scope.currentLine = 0
              ,$scope.currentPage = 0
              ,$scope.linePerPage = 15
              ,$scope.numPerLine = 5
              ,$scope.totalEmjojis = $scope.emojisList.length;

            $scope.isLastPage = function (page) {

              var emojisCountPerPage = $scope.numPerLine * $scope.linePerPage;
              var begin = ((page) * (emojisCountPerPage));
              var end = begin + emojisCountPerPage;

              if ( end <= $scope.emojisList.length) { return false}
              else { return true}
            }
            var goingUpOnLastPage = function (goPage) {
              return ($scope.isLastPage($scope.currentPage) && goPage > 0);
            }
            var goingdownOnFirstPage = function (goPage) {
              return ($scope.currentPage == 0 &&  goPage < 0);
            }
            var getLastPage = function () {
              var pageWidth = $scope.linePerPage * $scope.numPerLine;
              return Math.floor($scope.emojisList.length / pageWidth)+1 ;
            }

            var paginatorBucle = function (goPage){
              if ( goingUpOnLastPage (goPage) ) {
                $scope.currentPage = -1;
              }
              else if ( goingdownOnFirstPage (goPage) ){
                $scope.currentPage = getLastPage ();
              }
            }
            $scope.updateFilteredEmojis = function(goPage) {

              // If is the last or first return to the beggining or end
              paginatorBucle(goPage);
              ...
              var emojisCountPerPage = $scope.numPerLine * $scope.linePerPage;
              var begin = (($scope.currentPage) * (emojisCountPerPage));
              var end = begin + emojisCountPerPage;
              var emojisPerPage = ($scope.emojisList.slice(begin, end));

              // Then the lines for every page
              $scope.currentLine = 0;
              for ( x = 0 ; x < $scope.linePerPage ; x++){
                var beginLine = (($scope.currentLine) * $scope.numPerLine);
                var endLine = beginLine + $scope.numPerLine;
                $scope.filteredEmojis.push(emojisPerPage.slice(beginLine, endLine));
                $scope.currentLine += 1;
              }



            };$scope.updateFilteredEmojis (0);
